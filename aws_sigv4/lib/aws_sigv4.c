...
void get_signing_key(aws_sigv4_params_t* sigv4_params, aws_sigv4_str_t* signing_key)
{
  ...
  /* get HMAC SHA256 */
  HMAC(EVP_sha256(), key.data, key.len, msg.data, msg.len, signing_key->data, &signing_key->len);
...
int aws_sigv4_sign(aws_sigv4_params_t* sigv4_params, aws_sigv4_header_t* auth_header)
{
  ...
  get_string_to_sign(&sigv4_params->x_amz_date, &credential_scope, &canonical_request, &string_to_sign);
  ...
  /* get HMAC SHA256 */
  HMAC(EVP_sha256(), signing_key.data, signing_key.len, string_to_sign.data, string_to_sign.len, signed_msg.data, &signed_msg.len);
